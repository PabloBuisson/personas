...
export default function CreateProjectCard({ id }: { id: any }) {
  return (
    <li
      ...
      key={id}
    >
      <article className="h-full flex justify-center items-center text-purple-800 border-[0.15em] border-white relative">
        ...
        <Link
          className="relative p-1 flex flex-col grow h-full min-h-[20ch] min-w-[30ch]"
          href={`/projects/new`}
        >
          <div className="absolute flex justify-center items-center top-0 translate-y-[-50%] left-1/2 translate-x-[-50%] bg-purple-25 w-16 h-16 rounded-full border-[0.125em] border-purple-100">
          ...
          <div className="-z-50 absolute top-[-0.5rem] left-[-0.5rem] w-[calc(100%+1rem)] h-[calc(100%+1rem)] bg-purple-100 [clip-path:polygon(2rem_0%,_calc(100%-2rem)_0%,_100%_2rem,_100%_calc(100%-2rem),_calc(100%-2rem)_100%,_2rem_100%,_0%_calc(100%-2rem),_0%_2rem)] noisy-background"></div>
        </Link>
      </article>
    </li>
  );
